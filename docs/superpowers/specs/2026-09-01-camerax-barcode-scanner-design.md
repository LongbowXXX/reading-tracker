...
「上段のバーコードに向けてください」という常時文言を表示する。
下段を読んでも無反応になるため、なぜ確定しないのかを利用者に伝える必要がある。

**走査枠は実装していない（意図的な見送り）。** 当初この設計は文言と併せて走査枠を挙げていたが、
枠が実際に必要かどうかは、本を片手に持って上段へ狙いを付けるという実地の動作でしか判断できない。
枠の位置や大きさを机上で決めると、合わない枠に利用者を従わせる作りになりかねない。
ML Kit は画面内のどこにあるバーコードも読むため、枠は読み取り精度ではなく狙いの補助でしかない。
**実機確認の項目として「狙いを付けるのに枠が要るか」を残し、要ると分かった時点で追加する**
（`specs/001-reading-shelf-record/contracts/barcode-scanner.md` の実機確認項目に対応する行がある）。
