...
6. **オンデバイス AI の可用性は永続化しない**（FR-032, Issue #9）。判定は起動のたびに行い、端末側の更新で結果が変わりうるため、保存された値は次の起動時点で正しいとは限らない。したがって可用性・モデルの取得状況を表す列やテーブルを追加せず、判定結果は起動ゲートの画面状態としてのみ保持する（[contracts/ai-availability.md](./contracts/ai-availability.md)）。
