...
**全体のタイムアウト**: 2経路を**直列に**実行し、`lookup` の呼び出し開始からの経過時間が 6 秒を超えた時点で打ち切る。打ち切り時は、その時点までに得られた結果（`NotFound` のみなら `NotFound`、1件でも `Unavailable` があれば `Unavailable`）を返す。1経路あたりの 3 秒は各経路の上限であり、6 秒は全体の上限。両者は独立に適用される（例: 第1経路が 3 秒でタイムアウトした場合、第2経路には残り 3 秒が与えられる）。SC-001（記録完了 30 秒以内）に対する余裕を残すための値。
