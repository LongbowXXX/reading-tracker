...
| A-7 | `status()` が契約に反して例外を投げる | 非対応と同じ状態へ写し、文言のみ「確認できませんでした」とする（FR-033）。**防御的な経路**であり、契約どおりの実装では通らない（下記の注記を参照） |
| A-8 | 再試行を選ぶ | 確認中へ戻り、`status()` を再度呼ぶ（FR-032） |
| A-9 | 開発ビルドで非対応 | 「非対応のまま続行」で本体へ進める。配布ビルドではこの導線が無い（FR-035） |

**A-7 の位置づけ（2026-09-05, 実装時に確認）**: 契約表のとおり `status()` は例外を投げず、
判定できなかった場合も `UNSUPPORTED` を返す。したがって **ML Kit が例外を投げた場合、実機で出る文言は
「この端末は非対応です」であり、「確認できませんでした」は通常観測されない**。
A-7 はポートの実装が契約に反した場合にもゲートが壊れないための防御であり、
ユニットテストで固定するが、実機の確認項目としては扱わない（[quickstart.md](../quickstart.md) 4.6）。
両者で利用者に取れる行動は変わらないため、この扱いで FR-033 を満たす。
